...
from airbnb import compare_weekly_history, extract_daily_wow, report_conversion, summarize_conversion
...
def test_report_avoids_pricelabs_rule_recommendations(tmp_path: Path) -> None:
    ...
    assert "pricelabs rule change" not in text
...
def test_report_does_not_add_forbidden_airbnb_metric_fields() -> None:
    ...
    combined_columns = set(summarize_conversion.SUMMARY_COLUMNS) | set(extract_daily_wow.COLUMNS)
