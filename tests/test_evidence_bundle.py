...
def test_core_files_are_copied_and_labeled_core(tmp_path: Path) -> None:
    ...
    assert all(entry["source_of_truth_type"] == "core" for entry in manifest["files"])
    assert all(entry["copied"] for entry in manifest["files"])
...
def test_high_priority_optional_files_are_copied_only_when_priority_is_high(tmp_path: Path) -> None:
    ...
    assert all(entry["source_of_truth_type"] == "core" for entry in medium_manifest["files"])
    ...
    assert {"core", "diagnostic", "configuration"} == source_types
...
def test_missing_high_priority_optional_files_do_not_fail(tmp_path: Path) -> None:
    ...
    assert manifest["status"] == "complete"
    assert manifest["missing_required_files"] == []
    assert len(manifest["missing_optional_files"]) == 4
