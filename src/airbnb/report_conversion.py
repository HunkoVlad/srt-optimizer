...
def weekly_signal_lines(row: dict[str, str], history_rows: list[dict[str, str]] | None = None) -> list[str]:
    ...
    fields = [
        ...
        ("Estimated relevant searches", "estimated_relevant_searches", "relevant_searches_wow_change"),
        ("Estimated relevant searches/day", "estimated_relevant_searches_per_day", "relevant_searches_wow_change"),
...
def render_report(
    ...
    lines = [
        ...
        *weekly_signal_lines(row, history_rows or []),
        "",
        "## Relevant Search Demand Diagnostic",
        "",
        f"- Benchmark type: {row.get('benchmark_type', '-') or '-'}",
        f"- Relevant searches vs benchmark: {row.get('relevant_searches_vs_trailing_benchmark_pct', '-') or '-'}%",
        f"- Search-to-listing conversion vs benchmark: {row.get('search_to_listing_conversion_vs_benchmark_pct', '-') or '-'}%",
        f"- Listing-to-booking conversion vs benchmark: {row.get('listing_to_booking_conversion_vs_benchmark_pct', '-') or '-'}%",
        f"- Market demand status: {row.get('market_demand_status', '-') or '-'}",
        f"- Visibility status: {row.get('visibility_status', '-') or '-'}",
        f"- Search card status: {row.get('search_card_status', '-') or '-'}",
        f"- Listing conversion status: {row.get('listing_conversion_status', '-') or '-'}",
        f"- Diagnostic category: {row.get('airbnb_diagnostic_category', '-') or '-'}",
        "- Diagnostic only; Airbnb search demand and conversion classification must not create PriceLabs rule recommendations by itself.",
        "",
